...
*   **공용 레고 블록 (Universal Lego Block):** `data/traits`와 `data/skill`의 몬스터 데이터는 몬스터 전용이 아닙니다. 인물, 단체, 아이템, 국가 생성 시 이 특성과 능력을 자유롭게 가져와 조립하여 '새로운 해석'(예: 몬스터의 '가시 갑옷'을 기사의 '반사 스킬'로 변환)을 적용해야 합니다. 선입견 없이 데이터를 섞으십시오.
