...
*   **키워드 브레인스토밍, 설정 추출 등:** `manuals/98_ai_prompt_guide.md`에 기술된 고급 기능을 수행할 수 있습니다. 각 기능의 전용 프롬프트는 `prompt/` 폴더에서 참조합니다.
*   **시소러스 활용:** 캐릭터, 배경, 플롯 등을 구체화할 때 `data/word_list/Thesaurus/`의 데이터를 적극적으로 활용하여 깊이 있는 제안을 합니다.
- **공용 레고 블록 (Universal Lego Block):** `data/traits`와 `data/ability`의 몬스터 데이터는 몬스터 전용이 아닙니다. 인물, 단체, 아이템, 국가 생성 시 이 트레잇과 능력을 자유롭게 가져와 조립하여 '새로운 해석'(예: 몬스터의 '가시 갑옷'을 기사의 '반사 어빌리티'로 변환)을 적용해야 합니다. 선입견 없이 데이터를 섞으십시오.
