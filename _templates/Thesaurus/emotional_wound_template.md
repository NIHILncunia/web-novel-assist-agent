# emotional_wound_template.md

> **작성일:** 2026-01-08
> **수정일:** 2026-01-08

## **상처 (Wound Name)**

> 일러두기 (NOTES) 내용

- **구체적 상황 (EXAMPLES)**:
    - 구체적인 사건 예시들. 불릿으로 나열.
- **훼손 당하는 욕구 (BASIC NEEDS OFTEN COMPROMISED)**:
    - 안전, 사랑, 존중 등 손상되는 욕구. 쉼표로 나열.
- **생길 수 있는 잘못된 믿음 (FALSE BELIEFS)**:
    - 트라우마로 인해 갖게 된 왜곡된 신념. 불릿으로 나열.
- **가질 수 있는 두려움 (FEARS)**:
    - 공포증이나 회피 대상. 불릿으로 나열.
- **가능한 반응과 결과들 (POSSIBLE RESPONSES AND RESULTS)**:
    - 행동 패턴이나 결과. 불릿으로 나열.
- **형성될 수 있는 성격 트레잇 (PERSONALITY TRAITS THAT MAY FORM)**:
    - 방어기제로서 형성되는 성격 트레잇. 불릿으로 나열.
- **상처가 악화될 수 있는 계기 (TRIGGERS THAT MIGHT AGGRAVATE THIS WOUND)**:
    - 상처를 악화시킬 수 있는 상황. 불릿으로 나열.
- **상처를 직면하고 극복할 기회 (OPPORTUNITIES TO FACE OR OVERCOME THIS WOUND)**:
    - 상처를 극복할 수 있는 기회. 불릿으로 나열.
