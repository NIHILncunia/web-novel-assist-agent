---
name: create_trait
description: Extracts and classifies new Traits (특성) for characters, monsters, or objects. It analyzes the request to select one of the 14 standard categories (e.g., Physical, Mind, Origin) and appends the data to the corresponding `data/traits` file.
---

# 기능: 특성 생성 (`create_trait`)

이 **skills create_trait**는 캐릭터, 몬스터, 사물을 위한 **특성 (Trait)**을 생성하는 데 사용됩니다. '특성'이란 '무엇을 하는가(Ability)'가 아니라 '무엇인가(Identity)'를 정의하는 선천적 자질, 생리적 특징, 배경 요소를 의미합니다.

## 1. 분류 기준 (Classification Criteria)

데이터가 특성인지 능력인지 먼저 판단하십시오:
- **특성 (Trait):** 선천적, 생리적, 불변(대체로), 배경 설정. 예: '날개', '강철 피부', '용의 혈통'.
- **능력 (Skill):** 후천적, 기술적, 행동 가능, 소모성. 예: '비행 숙련', '철갑화 기술', '화염구'.

**만약 어빌리티(Ability)라면 **skills create_ability**를 대신 사용하십시오.**

## 2. 카테고리 선정 (14 Categories)

새로운 특성에 가장 적합한 **단 하나의 카테고리**를 선택하십시오. 이것이 저장될 파일(`data/traits/XX_Category.md`)을 결정합니다.
...
| **13** | **경영 (Management)** | 행정, 경제, 인프라 운영 능력. | `data/traits/13_경영.md` |

## 3. 실행 단계 (Execution Steps)

1.  **요청 분석:** 사용자가 요청한 특성의 핵심 컨셉을 파악합니다.
