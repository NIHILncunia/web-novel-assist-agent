...
### 7. 어빌리티 세부 리스트 분할 및 체계 개편 (진행 중)
- **목표:** `[권역:원천]-[계통]` 단위로 세분화된 어빌리티 리스트 파일 생성 및 신규 권역/형태 반영.
- **이유:** 기존의 권역별 통파일(`마법.md` 등)보다 더 정밀한 참조(예: '화염 방출계'만 조회)를 가능하게 하고, 최신화된 분류 체계를 유지하기 위함.
- **작업 내용:**
    - [x] **생산 권역 전면 리팩토링:** 4계통(채집, 가공, 제작, 조성) 및 12형태 체계 정립 및 데이터 이관 완료. (2026-02-03)
    - [ ] **[공연] 및 [화술] 관련 스킬 체계 통합:**
        - **화술 (Eloquence):** `[전술] - [교섭]` 계통으로 통합. (설득, 기만, 선동 형태 활용)
        - **공연 (Performance):** 
            - 신체적 기예: `[물리] - [기교]` 계통에 **[연행 (Performance)]** 형태 신설 및 통합.
            - 정신적 감흥: `[정신] - [간섭]` 계통의 **[동조]** 또는 **[매료]** 형태 활용.
    - [ ] **생활형 어빌리티(Life Skills) 대폭 확장:** 전투 외 일상, 서비스, 상업 분야에서 활용 가능한 어빌리티 20종 이상 추가 (청결, 보존, 고속 연산, 아공간 창고 등).
    - [ ] 스크립트 실행 환경 점검 (`uv run` 호환성 확인).
    - [ ] `scripts/split_ability_lists.py` 실행하여 `data/ability/detailed_lists/` 폴더의 나머지 권역 파일 생성 및 검증.

---

## ✅ 완료된 작업

### 생산 권역 체계 정립 및 데이터 리팩토링 (2026.02.03 완료)
- [x] **분류 체계 개편:** [생산] 권역을 자원 순환 구조에 따라 4계통(채집, 가공, 제작, 조성)으로 재정의.
- [x] **데이터 이관:** `data/ability/생산.md` 및 `detailed_lists/생산_*.md` 파일 12종 리팩토링 완료.
- [x] **가이드 동기화:** `manuals/99-1_ability_syntax.md` 및 `data/ability/어빌리티.md`에 새로운 생산 체계 반영 완료.
